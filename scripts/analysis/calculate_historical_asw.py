...
def get_pending_dates(engine, force=False):
    ...
    with engine.connect() as conn:
        if force:
            query = text("SELECT DISTINCT trade_date FROM bond_data WHERE trade_date >= '2026-01-01' ORDER BY trade_date DESC")
            result = conn.execute(query)
            return [row[0] for row in result]
        else:
            # Only dates not in ASW_data and from 2026 onwards
            query = text("""
...
def fetch_ois_data(conn, trade_date):
    """
    Fetch OIS data for a specific date using DatabaseManager.
    """
    db = DatabaseManager()
    date_str = trade_date.strftime("%Y-%m-%d")
    rows = db.get_ois_data(start_date=date_str, end_date=date_str)
    return [{'tenor': row['tenor'], 'rate': float(row['rate'])} for row in rows]
