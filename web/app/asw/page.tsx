...
import { fetcher, ASWCurveResponse, QuickDatesResponse } from '@/lib/api';
import ASWChart from '@/components/ASWChart';
import { Loader2, Plus, X, Calendar, TrendingUp, RefreshCw, ZoomIn, ArrowLeft, BarChart2 } from 'lucide-react';
...
export default function ASWPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        setLoading(true);
        // ASWデータがある日付を取得するためにテーブル指定
        const qd = await fetcher('/api/quick-dates?table=ASW_data');
        setQuickDates(qd);
        if (qd.latest && !initialized.current) {
  ...
  const addDateToChart = async (date: string) => {
    ...
    // 現在のdatasetsを直接参照してチェック
    if (datasets.some(ds => ds.date === date)) return;

    setLoading(true);
    setError(null);
    try {
      const resp: ASWCurveResponse = await fetcher(`/api/asw-data/${date}`);
      ...
      const newDataset: Dataset = {
        date: resp.date,
        data: resp.data,
        color: '' // setDatasets内で設定
      };
      
      setDatasets(prev => {
        // APIから返ってきた実際の日付で重複チェックを行う (最重要)
        if (prev.some(ds => ds.date === newDataset.date)) return prev;
        
        const updated = [...prev, newDataset];
        return updated.map((ds, idx) => ({
          ...ds,
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/50 p-6 rounded-2xl border border-white/20 shadow-sm">
          <div>
            ...
            <p className="text-slate-800 font-medium text-lg mt-2">
              複数の日付を追加して、ASW (Asset Swap Spread) の変動を比較分析します。
            </p>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden relative min-h-[650px] pb-6">
            ...
            <div className="bg-slate-50 border-t border-slate-100 px-8 py-5">
                <div className="flex flex-wrap gap-x-10 gap-y-2 text-sm text-slate-600 font-semibold">
                    ...
                    <p className="flex items-center gap-2">Type: ASW (Act/365)</p>
                </div>
            </div>
        </div>
      </div>
