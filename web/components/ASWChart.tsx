...
const ASWChart = ({ datasets, minMaturity = 0, maxMaturity = 40 }: ASWChartProps) => {
  ...
  // カスタムツールチップをコンポーネント内に移動して datasets にアクセスできるようにする
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const closestPoints = datasets.map(ds => {
        ...
        // その最小距離にある銘柄をすべて取得（同じ残存に複数ある場合に対応）
        // 誤差を考慮して 0.0001 未満のものを同一とみなす
                ...
                {group.bonds.map((bond, bIdx) => (
                  <div key={`${gIdx}-${bIdx}`} className="flex flex-col border-l-2 pl-3" style={{ borderColor: group.color }}>
                    <div className="flex justify-between items-center mb-0.5">
                      ...
                      <span className="font-black text-base" style={{ color: group.color }}>{bond.asw.toFixed(3)}%</span>
                    </div>
                    ...
                    <div className="text-[10px] text-slate-400 grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1 border-t border-slate-50 pt-1">
                      ...
                      <span className="text-right">Yield: {bond.bond_yield.toFixed(3)}%</span>
                      <span>Swap: {bond.swap_rate.toFixed(3)}%</span>
                    </div>
                  </div>
                ))}
              </div>
