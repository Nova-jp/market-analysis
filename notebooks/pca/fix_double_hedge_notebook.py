 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "- Let $v_{L,1}, v_{L,2}$ be the PC1 and PC2 loadings of the Long bond.\n",
    "- Let $v_{S,1}, v_{S,2}$ be the PC1 and PC2 loadings of the Short bond.\n",
    "- Portfolio Exposure $E = [v_{L,1}-v_{S,1}, v_{L,2}-v_{S,2}]^T$.\n",
    "- We solve the linear system for quantities $q_7$ and $q_{10}$:\n",
    "  $$\begin{bmatrix} v_{7,1} & v_{10,1} \\ v_{7,2} & v_{10,2} \end{bmatrix} \begin{bmatrix} q_7 \\ q_{10} \end{bmatrix} = -\begin{bmatrix} E_1 \\ E_2 \end{bmatrix}$$ \n",
    "- This ensures the net exposure to both PC1 and PC2 is zero."
   ]
  },
  {
   "cell_type": "code",
