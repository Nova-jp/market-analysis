...
def check_data_availability(engine, target_date):
    ...
    with engine.connect() as conn:
        ...
        # Check irs_data (OIS) using DatabaseManager
        ois_data = db.get_ois_data(start_date=target_date_str, end_date=target_date_str)
        ois_count = len(ois_data)
...
def main():
    ...
    # TONA (OIS) データの参照期間を 2026-01-01 以降に限定
    if target_date < date(2026, 1, 1):
        logger.info(f"ASW calculation is only supported for dates from 2026-01-01 (target: {target_date}). Skipping.")
        sys.exit(0)
