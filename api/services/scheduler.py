...
from api.services.jsda_volume import JSDAVolumeService
import jpholiday
...
class SchedulerService:
    ...
    def collect_evening_data(self) -> Dict[str, Any]:
        """
        夕方の統合データ収集を実行 (JSDA, 入札結果, IRS)
        21:00の実行を想定
        """
        logger.info("=== Starting evening combined data collection ===")
        results = {
            "status": "success",
            "message": "Evening combined collection completed",
            "details": {}
        }

        # 1. JSDA, BOJ, MOF(Sync) 収集 (既存の collect_data)
        try:
            logger.info("Step 1: Running daily JSDA/BOJ/MOF collection...")
            jsda_result = self.collect_data()
            results["details"]["daily_collection"] = jsda_result
            if jsda_result["status"] == "error":
                results["status"] = "partial_error"
        except Exception as e:
            logger.error(f"Daily collection failed: {e}")
            results["details"]["daily_collection"] = {"status": "error", "message": str(e)}
            results["status"] = "partial_error"

        # 2. 入札結果収集 (DailyAuctionCollector)
        try:
            logger.info("Step 2: Checking today's auctions...")
            from pipeline.fetchers.mof import DailyAuctionCollector, AuctionCalendarCollector
            today = date.today()
            calendar_collector = AuctionCalendarCollector()
            today_auctions = calendar_collector.get_auctions_for_date(today)
            
            if today_auctions:
                logger.info(f"Today's auctions found ({len(today_auctions)}). Collecting results...")
                auction_collector = DailyAuctionCollector()
                auction_result = auction_collector.collect_for_date(today)
                results["details"]["auction"] = {"status": "success", "data": auction_result}
            else:
                logger.info("No auctions scheduled for today. Skipping.")
                results["details"]["auction"] = {"status": "skipped", "message": "No auctions today"}
        except Exception as e:
            logger.error(f"Auction collection failed: {e}")
            results["details"]["auction"] = {"status": "error", "message": str(e)}
            results["status"] = "partial_error"

        # 3. IRSデータ収集 (IRSSchedulerService)
        try:
            logger.info("Step 3: Collecting IRS data...")
            from api.services.irs import IRSSchedulerService
            irs_service = IRSSchedulerService()
            irs_result = irs_service.collect_data()
            results["details"]["irs"] = irs_result
            if irs_result["status"] == "error":
                results["status"] = "partial_error"
        except Exception as e:
            logger.error(f"IRS collection failed: {e}")
            results["details"]["irs"] = {"status": "error", "message": str(e)}
            results["status"] = "partial_error"

        if results["status"] == "partial_error":
            results["message"] = "Evening collection completed with some errors"
        
        logger.info(f"=== Evening combined data collection finished. Status: {results['status']} ===")
        return results
