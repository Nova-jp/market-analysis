...
from core.models.schemas import QuickDatesResponse, DateSearchResponse, validate_date_format
    ...
    try:
        ...
        if len(q) == 10:
            ...
            result = await db_manager.get_bond_data(
                trade_date=q, limit=1, table_name=table
            )
            if result["success"]:
        ...
        # 部分入力の場合
            ...
            result = await db_manager.get_bond_data(
                start_date=q, order='desc', limit=100, table_name=table
            )
            if result["success"]:
...
async def get_quick_dates(
    ...
    try:
        ...
        where_clause = ""
        if table == "ASW_data":
            where_clause = "WHERE trade_date >= '2026-01-01'"
            
        sql = f'SELECT DISTINCT trade_date FROM "{table}" {where_clause} ORDER BY trade_date DESC LIMIT 30'
