...
def get_calendar_collector():
    ...
    if _calendar_collector is None:
        from pipeline.fetchers.mof import AuctionCalendarCollector
...
def verify_cloud_scheduler_request(
    ...
    Returns:
        bool: 検証成功時True
    """
    # 開発/デバッグモード（本番環境では完全無効）
    if os.getenv("ALLOW_SCHEDULER_DEBUG", "").lower() == "true":
        from core.config import settings
        if settings.is_production:
            logger.error(
                "ALLOW_SCHEDULER_DEBUG=true is set but environment is 'production'. "
                "This flag is blocked in production for security reasons."
            )
        else:
            logger.warning("Scheduler debug mode enabled - authentication bypassed (non-production only)")
            return True
...
async def macro_daily_data_collection(
    ...
    Cloud Schedulerから毎日07:10に呼び出される
    """
...
async def daily_data_collection(
    ...
    try:
        result = await run_in_threadpool(scheduler_service.collect_data)
...
async def auction_data_collection(
    ...
    try:
        ...
        # 入札結果収集
        from pipeline.fetchers.mof import DailyAuctionCollector
...
@router.post("/api/scheduler/evening-combined-collection")
async def evening_combined_collection(
    request: Request,
    x_cloudscheduler: Optional[str] = Header(None, alias="X-CloudScheduler"),
    user_agent: Optional[str] = Header(None, alias="User-Agent")
):
    """
    夕方の統合データ収集エンドポイント (JSDA, 入札結果, IRS)
    Cloud Schedulerから毎日21:00に呼び出される
    """
    if not verify_cloud_scheduler_request(request, x_cloudscheduler, user_agent):
        raise HTTPException(
    ...
    logger.info("=" * 60)
    logger.info("Evening combined data collection triggered by Cloud Scheduler")
    logger.info(f"Timestamp: {datetime.now().isoformat()}")
    logger.info("=" * 60)

    try:
        # 外部通信を含むため threadpool で実行
        result = await run_in_threadpool(scheduler_service.collect_evening_data)
        return result
    except Exception as e:
        error_msg = f"Unexpected error in Evening Combined collection: {str(e)}"
        logger.error(error_msg, exc_info=True)
        raise HTTPException(status_code=500, detail=error_msg)
...
async def international_transactions_collection(
    ...
    try:
        from pipeline.fetchers.mof.international_transactions_collector import InternationalTransactionsCollector
