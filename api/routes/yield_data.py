...
from core.models.schemas import (
    YieldCurveResponse, BondYieldData, SwapYieldData, SwapCurveResponse,
...
async def get_yield_data(
    ...
    try:
        ...
        # データベースから取得
        result = await db_manager.get_bond_data(trade_date=date, limit=1000)
...
async def get_asw_data(
    ...
    if date < "2026-01-01":
        return ASWCurveResponse(
            date=date,
            data=[],
            message="ASW data is only available from 2026-01-01"
        )
    try:
        # 1. Try to get accurate data from ASW_data
        db_result = await db_manager.get_bond_spreads(trade_date=date)
        ...
        logger.error("ASW calculation error", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
