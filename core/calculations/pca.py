...
class PCAService:
    ...
    def _get_cache_path(self, end_date: str, lookback_days: int) -> str:
        ...
        filename = f"pca_cache_{d_str}_{lookback_days}.joblib"
        # パストラバーサル対策: キャッシュディレクトリ外への逸脱を防ぐ
        path = os.path.realpath(os.path.join(self.cache_dir, filename))
        if not path.startswith(os.path.realpath(self.cache_dir)):
            raise ValueError(f"Invalid cache path: {path}")
        return path

    def save_cache(self, end_date: str, lookback_days: int, data: Dict):
        """分析結果をキャッシュに保存 (joblib使用)"""
    ...
    def get_swap_data(self, product_type: str = 'OIS', limit_days: int = 300) -> pd.DataFrame:
        ...
        """
        try:
            # 2026年以降のデータを取得 (DatabaseManager側でフィルタ適用)
            rows = self.db_manager.get_ois_data(product_type=product_type)
            if not rows:
                return pd.DataFrame()
            
            df = pd.DataFrame(rows)
            # 最新からlimit_days分の取引日を抽出
            unique_dates = sorted(df['trade_date'].unique(), reverse=True)[:limit_days]
            df = df[df['trade_date'].isin(unique_dates)]
            
            # 1Y, 10Yなどのテナーのみに限定し、数値化
            df = df[df['tenor'].str.endswith('Y')].copy()
